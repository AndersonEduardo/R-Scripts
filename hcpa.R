  ...
  for(j in 1:length(sampleSizes)){
    for(current_vies_level in 1:vies_levels){
      tryCatch({
        ...
        values(biasLayerBGptsKDE)[which(values(biasLayerBGptsKDE)<=0.2)] = NA
        
        
        ##amostra de pontos
        occPoints = dismo::randomPoints(mask=biasLayerBGptsKDE, n=sampleSizes[j]) #sorteando pontos da distribuicao modelada
        rm(SpDistAC) ##teste do bug persistente
        ...
        for (iter in 1:3){
          ...
          curent_occData_train = subset( x=myBiomodData@coord[!is.na(myBiomodData@data.species),], subset=kfold(myBiomodData@coord[!is.na(myBiomodData@data.species),], k=3)!=1 ) #occ para teste
          ##curent_bgData_train = myBiomodData@coord[is.na(myBiomodData@data.species),] #bg para train
          ...
          curent_bgData_test = myBiomodData@coord[is.na(myBiomodData@data.species),] #bg para teste
        ...
        statResultsSDMnormal = rbind(statResultsSDMnormal,
                                     data.frame(SDM='normal', sampleSize=sampleSizes[j], biaslevel=current_vies_level, evaluationScores))
        ...
        if ( length(nonBiomod) > 0 ){
          ...
          for (model_i in modelNamesAdjusted){ #rodando predict para cada um dos modelos indentificados
            for (repl_i in 1:3){ #replicas para mapear areas de incerteza
              ##dataset
              curent_occData_test = subset( x=myBiomodData@coord[!is.na(myBiomodData@data.species),], subset=kfold(myBiomodData@coord[!is.na(myBiomodData@data.species),], k=3)==1 ) #occ para teste
              ##predicao espacial
              currentSDM = SDMlist[[model_i]] #modelo da iteracao atual
    ...
    for(current_vies_level in 1:vies_levels){
      tryCatch({     
        ...
        statResultsSDMimproved = rbind(statResultsSDMimproved,
                                       data.frame(SDM='improved', sampleSize=sampleSizes[j], biaslevel=current_vies_level, evaluationScores))
...
for(i in c('MAXENT.Phillips','CTA',  'GAM',  'GBM',  'GLM',  'MARS', 'RF')){
  print(paste(i, 
              ' ==>> SDMnormal = ', dim(statResultsSDMnormal[statResultsSDMnormal$Model.name==i,])[1], ' linhas e ', dim(statResultsSDMnormal[statResultsSDMnormal$model==i,])[2], ' colunas',
              ' // '))#,
