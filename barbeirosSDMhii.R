...
## humDens = raster('D:/Anderson_Eduardo/Distribuicao de barbeiros com interacao com humanos/Variaveis Climaticas/presente/usadas/humDens.asc')
...
predictorsRaw = crop(x=predictorsRaw, y=SOAextent)
...
for (sp_i in splist){
    ...
    tryCatch({
        ...
        setwd(paste(projectFolder,'/SDM outputs/resultados SDM com humanos',sep=''))
        ...
        if (!file.exists(paste(envVarFolder,'/presente/usadas/SDMhuman/',sp_i,sep=''))){
            dir.create(paste(envVarFolder,'/presente/usadas/SDMhuman/',sp_i,sep=''), recursive=TRUE)
        }
        
        writeRaster(x=predictors, filename=paste(envVarFolder,'/presente/usadas/SDMhuman/',sp_i,'/predictors_',sp_i,'.asc',sep=''), overwrite=TRUE, bylayer=TRUE, suffix=names(predictors))
        ...
        writeRaster(SDMpred, paste(sp_i,'Suitability.asc',sep=''), overwrite=TRUE)
        ##binario
        SDMpredBIN = SDMpred > thre
        ...
        jpeg(paste(projectFolder,'/SDM outputs/resultados SDM com humanos/',sp_i,'/',sp_i,'_OccData.jpg',sep=''), width=1000, height=600)
        par(mfrow=c(1,2))
        ##occ e background
        ...
        jpeg(paste(projectFolder,'/SDM outputs/resultados SDM com humanos/',sp_i,'/',sp_i,'_Suitability.jpg',sep=''), width=1000, height=500)
        ...
        write.csv(SDMevalOutput, paste(projectFolder,'/SDM outputs/resultados SDM com humanos/',sp_i,'/',sp_i,'_SDMeval.csv',sep=''), row.names=FALSE)
        
        
        ##salvando graficos do ENMeval
        
        
        jpeg(paste(projectFolder,'/SDM outputs/resultados SDM com humanos/',sp_i,'/',sp_i,'_SDMeval.jpg',sep=''), width=800)
        ...
        write.csv(var.importance(modelPars), paste(projectFolder,'/SDM outputs/resultados SDM com humanos/',sp_i,'/',sp_i,'_variablesImportance.csv',sep=''), row.names=FALSE)
        ...
        write.csv(tabRes, paste(projectFolder,'/SDM outputs/resultados SDM com humanos/tabOutputsSDMhuman.csv',sep=''), row.names=FALSE)
...
tabOutputsSdmH = read.csv(paste(projectFolder,'/SDM outputs/resultados SDM com humanos/tabOutputsSDMhuman.csv', sep=''), header=TRUE)
tabOutputsSdm = read.csv(paste(projectFolder,'/SDM outputs/resultados SDM sem humanos/tabOutputsSDMclim.csv', sep=''), header=TRUE, sep=';')
...
for (sp_i in splist){ #loop para coletar os dados salvos
    ...
    ## hvar = colSums(tabelaSDMhuman[,c(2:3)])
    ...
    if(match(sp_i, splist)==1){
        ...
        varImpSDMclim = data.frame(variable=paste('bio',seq(19),sep=''))
        varImpSDMhuman = data.frame(variable=c('humDens', paste('bio',seq(19),sep='')))
    }
    ...
    ##SDMclim
    idxSDMclim = match(tabelaSDMclim$variable, varImpSDMclim$variable)
    spsSDMclim = rep(0, 19)
    spsSDMclim[idxSDMclim] = tabelaSDMclim[,'permutation.importance']
    varImpSDMclim = cbind(varImpSDMclim, spsSDMclim)

    ##SDMhuman
    idxSDMhuman = match(tabelaSDMhuman$variable, varImpSDMhuman$variable)
    spsSDMhuman = rep(0, 20)
    spsSDMhuman[idxSDMhuman] = tabelaSDMhuman[,'permutation.importance']
    varImpSDMhuman = cbind(varImpSDMhuman, spsSDMhuman)
...
par(mar=c(13,3,1,10))
barplot(as.matrix(varImpSDMhuman[,-1]), col=c('black',rainbow(nrow(varImpSDMhuman)-1)), las=2, names.arg=gsub('_',' ',names(varImpSDMhuman)[-1]))
abline(h=0)
par(xpd=TRUE)
legend(18,100, c('Human density',gsub('predictors_bio', 'Bio ',  as.character(varImpSDMhuman$variable))[-1]), pch=15, col=c('black',rainbow(nrow(varImpSDMhuman)-1)))
dev.off()
...
SDMclimRich = raster(paste(projectFolder,'/SDM outputs/graficos/SDMclimAcumSuit.asc', sep=''))
SDMhumanRich = raster(paste(projectFolder,'/SDM outputs/graficos/SDMhumanAcumSuit.asc', sep=''))
