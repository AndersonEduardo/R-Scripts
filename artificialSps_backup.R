...
for (i in 1:length(caminhosCamadasTemp)){
    ...
    parametersHW <- formatFunctions(bioclim_01=c(fun='betaFun',p1=200,p2=295,alpha=1,gamma=1),bioclim_12=c(fun='betaFun',p1=2000,p2=3500,alpha=1,gamma=1)) #criando as respostas da especie Ã s variaveis ambientais

    ##funcoes especie de clima quente e seco
    parametersHD <- formatFunctions(bioclim_01=c(fun='betaFun',p1=200,p2=260,alpha=1,gamma=1),bioclim_12=c(fun='betaFun',p1=50,p2=1800,alpha=1,gamma=1)) #criando as respostas da especie Ã s variaveis ambientais
 
    ##funcoes especie de clima frio e seco
    parametersCD <- formatFunctions(bioclim_01=c(fun='betaFun',p1=50,p2=220,alpha=1,gamma=1),bioclim_12=c(fun='betaFun',p1=50,p2=1800,alpha=1,gamma=1)) #criando as respostas da especie Ã s variaveis ambientais
        ...
        for (j in 1:length(sampleSizes)){
            for (k in 1:NumRep){ #loop sobre o numero de replicas 
                tryCatch({
                    ...
                    names(backgroundPoints) = names(occPoints) #certificando que os nomes das colunas estÃ£o iguais (cuidado aqui...)
...
## Shoener's D e Hellinger X nÃºmero de camadas no SDMmultitemporal
...
par(mfrow=c(1,2))
plot(dataClamp[which(dataClamp$sdm=='multitemporal'),]$clamping ~ as.factor(dataClamp[which(dataClamp$sdm=='multitemporal'),]$kyr), ylim=c(0,15), xlab='Time (in kyr BP)',  ylab='South AmÃ©rica area (in %)', main='Multitemporal')
plot(dataClamp[which(dataClamp$sdm=='monotemporal'),]$clamping ~ as.factor(dataClamp[which(dataClamp$sdm=='monotemporal'),]$kyr), ylim=c(0,15), xlab='Time (in kyr BP)',  ylab='South AmÃ©rica area (in %)', main='Monotemporal')
dev.off()
...
plot(dataClamp[which(dataClamp$sdm=='monotemporal' & dataClamp$sp=='spCD' & dataClamp$sampleSize==50),]$clamping ~ as.factor(dataClamp[which(dataClamp$sdm=='monotemporal' & dataClamp$sp=='spCD' & dataClamp$sampleSize==50),]$kyr), ylim=c(0,15), xlab='',  ylab='South AmÃ©rica area (in %)', main='', col=rgb(0,0,0,alpha=0.5))
...
plot(dataClamp[which(dataClamp$sdm=='monotemporal' & dataClamp$sp=='spHW' & dataClamp$sampleSize==50),]$clamping ~ as.factor(dataClamp[which(dataClamp$sdm=='monotemporal' & dataClamp$sp=='spHW' & dataClamp$sampleSize==50),]$kyr), ylim=c(0,15), xlab='',  ylab='South AmÃ©rica area (in %)', main='', col=rgb(0,0,0,alpha=0.5))
