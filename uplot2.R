...
uplot2 = function(x, shape=NULL, niche.metric=NULL, legend=TRUE){
  ...
  if (is.null(shape) & is.null(niche.metric)){
    stop("Quando um shapefile não é fornecido o argumento 'niche.metric' deve ser especificado (as opções são: 'TSS' e 'ROC').")
  }
  
  if (length(niche.metric) > 1){
    stop("Quando um shapefile não é fornecido o argumento 'niche.metric' deve ser especificado com uma das opções: 'marginility' e 'ROC'.")
  }
  ...
  if( is.null(shape) ){
    ...
    ## plot(x$uniche.TSS)
    ## points(xx$uniche.TSS$PRCC$original, pch=20, cex=tempUncert, col=sensitivity)
    ## abline(h=0, col='red', lty=2)
    
    plot(seq(ncol(x$uniche.TSS$X)), apply( x$uniche.TSS$PRCC[,c('min. c.i.','max. c.i.')], 1, sum)/2, xaxt="n", xlab='Points', ylab='PRCC', main='TSS', ylim=c(-1,1), pch=20, cex=tempUncert-1.5, col=sensitivity)
    ...
    plot(seq(ncol(x$uniche.ROC$X)), apply( x$uniche.ROC$PRCC[,c('min. c.i.','max. c.i.')], 1, sum)/2, xaxt="n", xlab='Points', ylab='PRCC', main='ROC', ylim=c(-1,1), pch=20, cex=tempUncert-1.5, col=sensitivity)
